...
func Networkstatus_listbylocationHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		locationNameVal, ok := args["locationName"]
		if !ok {
			return mcp.NewToolResultError("Missing required path parameter: locationName"), nil
		}
		locationName, ok := locationNameVal.(string)
		if !ok {
			return mcp.NewToolResultError("Invalid path parameter: locationName"), nil
		}
		...
		url := fmt.Sprintf("%s/subscriptions/%s/resourceGroups/%s/providers/Microsoft.ApiManagement/service/%s/locations/%s/networkstatus%s", cfg.BaseURL, subscriptionId, resourceGroupName, serviceName, locationName, queryString)
		...
		var result models.NetworkStatusContract
...
func CreateNetworkstatus_listbylocationTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft_ApiManagement_service_serviceName_locations_locationName_networkstatus",
		...
		mcp.WithString("locationName", mcp.Required(), mcp.Description("Location in which the API Management service is deployed. This is one of the Azure Regions like West US, East US, South Central US.")),
		mcp.WithString("api-version", mcp.Required(), mcp.Description("Version of the API to be used with the client request.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Networkstatus_listbylocationHandler(cfg),
	}
}
